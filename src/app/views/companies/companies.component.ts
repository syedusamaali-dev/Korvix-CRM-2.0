...
export class CompaniesComponent implements OnInit {
  ...
  activeDetailTab: 'overview' | 'contacts' | 'deals' | 'documents' | 'timeline' = 'overview';
  ...
  get activeCompanies(): number { return this.companies.filter(c => c.status === 'Active').length; }
  get enterpriseCount(): number { return this.companies.filter(c => c.companySize === 'Enterprise').length; }
  get smbCount(): number { return this.companies.filter(c => c.companySize === 'SMB').length; }
  ...
  applyFilters(): void {
    this.filteredCompanies = this.companies.filter(company => {
      ...
      const matchesSize = this.selectedSize === 'All' || company.companySize === this.selectedSize;
  ...
  updatePagination(): void {
    this.totalPages = Math.ceil(this.filteredCompanies.length / this.pageSize) || 1;
    const start = (this.currentPage - 1) * this.pageSize;
    this.paginatedCompanies = this.filteredCompanies.slice(start, start + this.pageSize);
  }
  ...
  deleteSelected(): void {
    if (confirm(`Are you sure you want to delete ${this.selectedIds.size} company record(s)?`)) {
  ...
  exportCSV(): void {
    const headers = ['Company Name', 'Industry', 'Website', 'Phone', 'Country', 'Size', 'Status', 'Owner'];
    const rows = this.filteredCompanies.map(c => [
      `"${c.name}"`, `"${c.industry}"`, `"${c.website}"`, `"${c.phone}"`, `"${c.country}"`, c.companySize, c.status, `"${c.owner}"`
    ]);
